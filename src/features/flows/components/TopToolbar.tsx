          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <Input
              ...
              className={`dark:bg-transparent dark:border-gray-800 dark:text-gray-100 text-sm ${
                isDesktop ? 'max-w-xs h-8' : 'max-w-xs h-7'
              }`}
            />
          ...
          <div className="flex items-center gap-1.5">
            <Button
              ...
              className={`dark:bg-transparent dark:border-gray-800 dark:hover:bg-gray-900/50 dark:text-gray-300 border-gray-200 hover:bg-gray-50 px-2 ${
                isDesktop ? 'h-8 px-3' : 'h-7'
              }`}
            >
              <Play className={`${isDesktop ? 'h-4 w-4' : 'h-3 w-3'}`} />
            </Button>
            ...
            >
              {isSaving ? (
                <Loader2 className={`${isDesktop ? 'h-4 w-4' : 'h-3 w-3'} animate-spin`} />
              ) : (
                <Save className={`${isDesktop ? 'h-4 w-4' : 'h-3 w-3'}`} />
              )}
            </Button>

            {/* Desktop: adicionar botão de tela cheia */}
            {isDesktop && (
              <Button
                variant="outline"
                size="sm"
            ...
            {/* Substituir expandir por limpar todos */}
            <Button
              variant="outline"
              size="sm"
            ...
            >
              <Eraser className={`${isDesktop ? 'h-4 w-4' : 'h-3 w-3'}`} />
            </Button>
          </div>
        </div>
        ...
        <div className={`flex items-center gap-1 ${
          ...
          {nodeTypes.map((nodeType) => {
            ...
            return (
              ...
              >
                <IconComponent className={`${isDesktop ? 'h-4 w-4' : 'h-3 w-3'} ${nodeType.color}`} />
              </Button>
            );
          })}

          {/* Ferramentas adicionais para desktop/tablet */}
          {!isMobile && (
            <Button
              variant="outline"
              size="sm"
              ...
              className={`dark:bg-transparent dark:border-gray-800 dark:hover:bg-gray-900/50 dark:text-gray-300 border-gray-200 hover:bg-gray-50 p-0 ${
                isDesktop ? 'h-8 w-8' : 'h-6 w-6'
              }`}
            >
              <AlignJustify className={`${isDesktop ? 'h-4 w-4' : 'h-3 w-3'}`} />
            </Button>
          )}
