  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (user.role === 'super_admin') {
        ...
        metrics.activeFlows = flowsResult.data?.filter(f => f.is_active).length || 0;
        metrics.totalExecutions = executionsResult.data?.length || 0;
        metrics.completedExecutions = executionsResult.data?.filter(e => e.status === 'completed').length || 0;
        metrics.totalUsers = usersResult.data?.length || 0;
        metrics.totalClinics = usersResult.data?.filter(u => u.role === 'clinic').length || 0;
        ...
        const [flowsResult, executionsResult] = await Promise.all([
          ...
          supabase
            ...
            .in('flow_id', 
              (await supabase.from('flows').select('id').eq('created_by', user.id)).data?.map(f => f.id) || []
            )
        ]);
